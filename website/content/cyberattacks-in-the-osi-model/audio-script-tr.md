...
Fiziksel Katman (Physical Layer): Elektriksel, mekanik ve fiziksel bağlantıları yönetir. Bu katman, verinin iletimi için kullanılan kablo türleri, konektörler, fiziksel bağlantı noktaları ve veri sinyallerinin nasıl iletilip alınacağı gibi detayları içerir.
Veri Bağlantı Katmanı (Data Link Layer): Fiziksel bağlantı üzerinde hataları düzeltir ve güvenli bir iletişim sağlar. Veri paketlerini alır, parçalara ayırır (framing), her bir parçayı adresler (MAC adresiyle), hataları kontrol eder ve gerekirse düzeltir.
Ağ Katmanı (Network Layer): Veri paketlerinin iletilmesi için en uygun yolun belirlenmesini sağlar. İletim rotalarını yönetir ve ağdaki diğer cihazlarla bağlantıyı sağlar. IP adreslemesi ve yönlendirme gibi işlemler bu katmanda gerçekleşir.
Taşıma Katmanı (Transport Layer): Veri akışını kontrol eder ve veri bütünlüğünü sağlar. Verilerin doğru bir şekilde gönderilmesini, sıralanmasını ve hedefe ulaşmasını sağlar. Bu katmanda yaygın olarak kullanılan protokoller arasında TCP (Transmission Control Protocol) ve UDP (User Datagram Protocol) bulunur.
Oturum Katmanı (Session Layer): İki cihaz arasında oturumların oluşturulması, yönetilmesi ve sonlandırılmasından sorumludur. Oturum katmanı, veri transferi sırasında güvenlik, kimlik doğrulama ve veri senkronizasyonu gibi işlevleri gerçekleştirir.
...
<span class="render-badge">ÖNLEM</span>
<h3>Sniffing Savunması</h3>
<p>Sniffing saldırılarına karşı en etkin çözüm <strong>uçtan uca şifrelemedir</strong>. SSH, HTTPS, SFTP ve WPA3 gibi güvenli şifreli iletişim protokolleri kullanılmalı; düz metin (plaintext) protokoller (HTTP, FTP, Telnet) yasaklanmalıdır.</p>
</div>
</div>
...
<p>Saldırganın kendi ağ arayüz kartının (NIC) fiziksel adresini, ağdaki yetkili bir cihazın adresiyle değiştirerek kimlik doğrulama sistemlerini (örn. MAC Filtreleme) atlatmasıdır.</p>
</div>
<div class="render-card render-card-ssr">
<span class="render-badge">ARP</span>
<h3>ARP Poisoning (Zehirleme)</h3>
<p>LAN üzerindeki cihazlara sahte ARP yanıtları göndererek, hedef cihazların ağ trafiğinin saldırganın makinesi üzerinden akmasını (Man-in-the-Middle) sağlamaktır.</p>
</div>
<div class="render-card render-card-ssg">
<span class="render-badge">IP</span>
<h3>IP Spoofing (Sahtecilik)</h3>
<p>Gönderilen IP paketlerinin kaynak IP adresi alanını sahte veya başka bir yetkili makinenin IP'siyle değiştirerek paketin kaynağını gizleme veya yetki aşımı sağlama yöntemidir.</p>
</div>
</div>
...
<h3>TCP SYN Flood</h3>
<p>TCP üç yönlü el sıkışmasını istismar eder. Saldırgan sürekli SYN istekleri gönderir fakat gelen SYN-ACK yanıtlarına ACK ile cevap vermez, hedef sunucunun bağlantı tablosunu doldurarak DoS oluşturur.</p>
</div>
<div class="render-card render-card-ssr">
<span class="render-badge">TCP RESET</span>
<h3>TCP Reset Saldırısı</h3>
<p>Saldırgan iki cihaz arasındaki TCP bağlantısını koparmak amacıyla, bağlantıyı kullanan cihazlardan birine veya her ikisine RST biti set edilmiş sahte TCP paketleri gönderir.</p>
</div>
<div class="render-card render-card-csr">
<span class="render-badge">UDP FLOOD</span>
<h3>UDP Flood Saldırısı</h3>
<p>Saldırgan hedef cihazın rastgele portlarına çok sayıda UDP paketi gönderir. Sunucu her kapalı port için ICMP port unreachable hatası dönmek zorunda kalır ve bant genişliği tükenir.</p>
</div>
</div>
...
XML Injection Attack: Bu saldırısı, XML veritabanındaki verileri bozabilir ve web sitesinin güvenliğini tehdit edebilir. Veritabanında depolanan tüm hassas bilgilere erişebilir ve web sitesinde istedikleri değişiklikleri yapabilirler.
SQL Injection Attack: Bir giriş alanına kötü niyetli bir SQL ifadesi ekleyerek web sitelerine veya herhangi bir SQL veritabanına SQL enjeksiyon saldırısı gerçekleştirebilir.
DLL Injection Attack: Dinamik bağlantı kitaplığı (DLL) dosyası, Windows'ta belirli bir etkinliği gerçekleştirmek için bir dizi kod ve veri içeren bir kitaplıktır. Uygulamalar bu tür bir dosyayı, bu etkinliği gerçekleştirmeleri gerektiğinde yerleşik olmayan işlevsellik eklemek için kullanır. DLL enjeksiyonu, bir saldırganın bir uygulamayı hedef sürecin bir parçası olarak çalışan zararlı bir DLL dosyasını çağırması için kandırmasına olanak tanır.
...
Local File Inclusion (LFI): Saldırgan, web uygulamasının sunucusunda yer alan yerel dosyalara erişim sağlamak için kullanıcı tarafından sağlanan girişleri manipüle eder. Saldırgan, sunucuda bulunan hassas dosyalara (örneğin, konfigürasyon dosyalarına, kullanıcı kimlik doğrulama bilgilerine vb.) erişebilir ve bu bilgileri istismar edebilir.
Remote File Inclusion (RFI): Saldırgan, dışarıdaki uzak bir sunucuda bulunan dosyaları web uygulamasına dahil etmek için kullanıcı tarafından sağlanan girişleri manipüle eder. Saldırgan, hedef sunucuya zararlı bir dosya veya betik dahil ederek sistemde istenmeyen işlemler gerçekleştirebilir.
...
Stored (Kalıcı) XSS: Zararlı betikler, web uygulamasında kalıcı olarak saklanır ve her defasında hedef kullanıcılar tarafından görüntülenir. Bu tür saldırılar, forumlar, yorum alanları veya mesaj panoları gibi kullanıcı tarafından oluşturulan içeriği saklayan uygulamalarda yaygındır.
Reflected (Yansıtılan) XSS: Zararlı betikler, kullanıcının web uygulamasına gönderdiği istekle birlikte gönderilir ve geri dönen yanıtta hedef kullanıcı tarafından çalıştırılır. Bu tür saldırılar, arama formları veya URL parametreleri gibi kullanıcının sağladığı girişleri işleyen uygulamalarda ortaya çıkar.
DOM-based XSS: Bu tür XSS saldırıları, Document Object Model (DOM) üzerindeki güvenlik açıklarını hedef alır. Saldırganlar, web sayfasının DOM yapısını manipüle ederek zararlı betikleri hedef kullanıcının tarayıcısında çalıştırır.
