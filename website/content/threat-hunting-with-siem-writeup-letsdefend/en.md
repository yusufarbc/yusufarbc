...
layout: single
title: Threat Hunting with SIEM Writeup - LetsDefend
...
Creating solutions is a critical step in the threat hunting process. Cybersecurity analysts determine how potential threats might emerge based on existing data and threat intelligence, and they develop accordingly. These findings guide the detection of specific attacker behaviors or abnormal activities.
...
The MITER ATT&CK Framework systematically classifies the techniques and procedures used by attackers, guiding security analysts in the following areas:
...
Hypotheses should explain how a specific event or behavior can be detected. Well-defined formulas include the following components:
...
**Collect Login/Logout Logs:** Gather information about when the user is logged in and out of the system.
...
SIEM systems support threat hunting processes within specific reports and provide powerful tools for detecting suspicious admin account activity. The steps outlined in this lesson demonstrate how SIEM can be used to detect unauthorized access attempts to administrator accounts. The process helps organizations proactively manage their cybersecurity posture and become more resilient to potential threats.
...
We understand that 172.16.8 IPs are DMZ assets. Let's search for ssh fail events to these hosts. For this we need to search for sshd group and authentication\_failed event. When we write the necessary filters, we find the answer as 24 hits.
...
For the adm\_eric account, we need to find out from which country outside of Germany the successful login was made. for this we need to filter the account and achieve success.
...
If we search the GeoLocation.country\_name information in the log we find, we find the answer. Or you can search in the full log.
...
If we look at the full log content, we can determine that it was the root user.
