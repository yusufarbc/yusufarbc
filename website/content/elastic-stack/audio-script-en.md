...
[Table Start]
Rule Name: PS-Encoded-Cmd-Detection-001. Rule Logic (EQL/Threshold): process where process.name == "powershell.exe" and process.command_line : "EncodedCommand". Severity: High. Definition: Detect the execution of PowerShell with the --EncodedCommand parameter. MITRE Tactic: Execution. MITRE Technical ID: T1059.001.
Rule Name: VssAdmin-Shadow-Delete-002. Rule Logic (EQL/Threshold): process where process.name : "vssadmin.exe" and process.args : "delete" and process.args : "shadows". Severity: Critical. Definition: Detects deletion of shadow copies with vssadmin.exe. MITRE Tactic: Impact. MITRE Technical ID: T1490.
Rule Name: Net-User-Recon-003. Rule Logic (EQL/Threshold): process where (process.name == "net.exe" or process.name == "net1.exe") and process.command_line: ("user" or "users"). Severity: Medium. Definition: Detects account discovery activity using the net.exe tool.. MITRE Tactic: Discovery. MITRE Technical ID: T1087.
Rule Name: Port-Scan-Detection-004. Rule Logic (EQL/Threshold): count(destination.port) by source.ip > 20 in 1m. Severity: Medium. Definition: Connection from a single IP to many different ports.. MITRE Tactic: Discovery. MITRE Technical ID: T1046.
[Table End]
