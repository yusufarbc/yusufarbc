...
[Tablo Başlangıcı]
Kural Adı: PS-Encoded-Cmd-Detection-001. Kural Mantığı (EQL/Threshold): process where process.name == "powershell.exe" and process.command_line : "EncodedCommand". Ciddiyet: Yüksek. Tanım: PowerShell'in --EncodedCommand parametresi ile çalıştırılmasını tespit eder.. MITRE Taktik: Yürütme. MITRE Teknik ID: T1059.001.
Kural Adı: VssAdmin-Shadow-Delete-002. Kural Mantığı (EQL/Threshold): process where process.name : "vssadmin.exe" and process.args : "delete" and process.args : "shadows". Ciddiyet: Kritik. Tanım: vssadmin.exe ile gölge kopyaların silinmesini tespit eder.. MITRE Taktik: Etki. MITRE Teknik ID: T1490.
Kural Adı: Net-User-Recon-003. Kural Mantığı (EQL/Threshold): process where (process.name == "net.exe" or process.name == "net1.exe") and process.command_line : ("user" or "users"). Ciddiyet: Orta. Tanım: net.exe aracını kullanarak hesap keşfi aktivitesini tespit eder.. MITRE Taktik: Keşif. MITRE Teknik ID: T1087.
Kural Adı: Port-Scan-Detection-004. Kural Mantığı (EQL/Threshold): count(destination.port) by source.ip > 20 in 1m. Ciddiyet: Orta. Tanım: Tek bir IP'den çok sayıda farklı porta yapılan bağlantı girişimlerini tespit eder.. MITRE Taktik: Keşif. MITRE Teknik ID: T1046.
[Tablo Bitişi]
