...
Bölüm: EPP, EDR ve XDR Arasındaki Farklar
...
[Tablo Başlangıcı]
Özellik: Yönetim Tarzı. EPP (Endpoint Protection): Pasif / Kuralcı ("Kur ve Unut"). Politikalar merkezden basılır.. EDR (Endpoint Detection & Response): Aktif / Analitik. Sürekli veri takibi ve tehdit avcılığı (Hunting) gerektirir.. XDR (Extended Detection & Response): Hibrit / Otomatize. Farklı sistemlerin merkezi yönetim orkestrasyonudur..
Özellik: Alarm Tipi. EPP (Endpoint Protection): Binary (0/1). Tehdit ya vardır ya yoktur. Genellikle "Temizlendi" bilgisidir.. EDR (Endpoint Detection & Response): Konseptsel / Bağlamsal. Davranışın neden şüpheli olduğunu kanıtlarla sunar.. XDR (Extended Detection & Response): Bütünsel (Storyline). Saldırı zincirinin (Kill Chain) tamamını tek hikaye gibi sunar..
Özellik: Alarm Yönetimi. EPP (Endpoint Protection): Az sayıda alarm üretir ancak False Negative (kaçırma) oranı yüksektir.. EDR (Endpoint Detection & Response): Küçük şüpheli hareketleri de raporlar. "Alarm Yorgunluğu" (Alert Fatigue) yaratabilir.. XDR (Extended Detection & Response): Akıllı algoritmalarla farklı logları birleştirir, alarm sayısını %50-70 azaltır..
Özellik: Müdahale (Response). EPP (Endpoint Protection): Dosya karantinası, USB engelleme, sistem taraması.. EDR (Endpoint Detection & Response): Cihaz izole etme, süreci (process) öldürme, RAM dökümü alma, komut çalıştırma.. XDR (Extended Detection & Response): EDR aksiyonlarına ek olarak; ağda IP engelleme, bulut oturumunu kapatma, e-posta temizleme..
Özellik: İhtiyaç Duyulan Kaynak. EPP (Endpoint Protection): IT veya Sistem Yöneticisi tarafından kolayca yönetilebilir.. EDR (Endpoint Detection & Response): Uzman SOC analisti veya güvenlik ekibi gerektirir.. XDR (Extended Detection & Response): Üst seviye güvenlik mimarı veya MDR (Yönetilen Hizmet) desteği gerektirir..
[Tablo Bitişi]
...
Bölüm: Microsoft Defender for Endpoint
...
Bölüm: Kaspersky Endpoint Security for Business
...
Bölüm: NOC ve SOC Yakınsaması (Bütünleşik BT Operasyonları)
