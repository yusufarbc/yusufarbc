Bölüm: İşletim Sistemi Konsepti ve Dinozor Kitabı'nın İzinde
...
Bölüm Detayı: Neden Herkes Ona "Dinozor" Diyor?
...
Bölüm: İşletim Sistemi (Operating System)
...
Bölüm Detayı: İşletim Sistemi Yapısı
...
Bölüm Detayı: Kullanıcı Modu ve Çekirdek Modu Geçişi (User Mode vs. Kernel Mode)
...
Bölüm Detayı: İşletim Sistemi Görevleri
...
Bölüm Detayı: PCB(Process Control Block)
...
Bölüm Detayı: İşlemler Arası İletişim
...
Bölüm Detayı: İş Parçacıkları(Thread)
...
Bölüm Detayı: Multithreading Modelleri
...
Bölüm: CPU Zamanlama (CPU Scheduling)
...
Bölüm Detayı: İşlem Senkronizasyonu
...
Karşılıklı dışlama: işlem kritik bölümünde çalışıyorsa, başka hiçbir.
işlem kritik bölümde çalışamaz.
İlerleme: Hiçbir işlem kendi kritik bölümünde çalışmıyorsa ve bazı.
...
ertelenemez.
Sınırlandırılmış bekleme: Bir işlemden sonra diğer işlemlerin kritik.
böüme girmelerine izin verilen sayı üzerinde bir sınır veya limit
vardır.

Bu gereksinimler için Peterson Çözümü geliştirilmiştir.
...
Bölüm Detayı: Bankacı Algoritması (Banker's Algorithm)
...
Bölüm: Bellek Yönetimi (Memory Management)
...
Bölüm Detayı: Segmentasyon(Segmentation)
...
Bölüm Detayı: Sanal Bellek(Virtual Memory)
...
Bölüm Detayı: Sayfa Değiştirme Algoritmaları (Page Replacement Algorithms)
...
Bölüm: Disk Yönetimi (Mass Storage Management)
...
Bölüm Detayı: Disk Zamanlama(Disk Scheduling)
...
FAT (File Allocation Table): FAT dosya sistemi, Microsoft tarafından geliştirilen ve özellikle Windows işletim sistemlerinde kullanılan bir dosya sistemidir. FAT, dosyaları depolamak için bir dosya tablosu kullanır ve dosyaların disk üzerindeki yerini belirleyen bir dosya tahsis tablosuna sahiptir.

NTFS (New Technology File System): NTFS, Microsoft'un geliştirdiği ve Windows NT tabanlı işletim sistemlerinde yaygın olarak kullanılan bir dosya sistemidir. NTFS, gelişmiş güvenlik özellikleri, dosya şifreleme, dosya sıkıştırma, hata toleransı ve günlükleme gibi gelişmiş özellikler sunar.

HFS+ (Hierarchical File System Plus): HFS+, Apple'ın macOS işletim sistemlerinde kullanılan bir dosya sistemidir. HFS+, önceki HFS dosya sisteminin geliştirilmiş bir sürümüdür ve büyük dosya boyutları, günlükleme, dosya şifreleme ve Unicode desteği gibi özelliklere sahiptir.

ext4 (Fourth Extended File System): ext4, Linux işletim sistemlerinde kullanılan bir dosya sistemidir. ext4, önceki ext2 ve ext3 dosya sistemlerinin geliştirilmiş bir sürümüdür ve günlükleme, büyük dosya ve bölüm boyutları, daha hızlı veri erişimi ve daha iyi veri bütünlüğü sağlar.

APFS (Apple File System): APFS, Apple'ın macOS ve iOS işletim sistemlerinde kullanılan yeni bir dosya sistemidir. APFS, hızlı veri erişimi, yüksek performans, dosya şifreleme, yerin sıkıştırılması ve hata toleransı gibi özellikleri destekler.
...
Sequential Access: En basit erişim yöntemi sıralı erişimdir. Dosyadaki bilgiler sırayla, bir kayıt diğerinin ardından işlenir. Bu erişim şekli en yaygın olanıdır; örneğin, editörler ve derleyiciler dosyalara genellikle bu şekilde erişir.
Direct Access: Doğrudan erişim dosyalar, büyük miktarda bilgiye anında erişim için çok kullanışlıdır. Veritabanları genellikle bu türdendir. Belirli bir konuyla ilgili bir sorgu geldiğinde, hangi bloğun cevabı içerdiğini hesaplarız ve ardından istenilen bilgiyi sağlamak için doğrudan o bloğu okuruz.
Other Access Methods: Diğer erişim yöntemleri doğrudan erişim yönteminin üzerine inşa edilebilir. Bu yöntemler genellikle dosya için bir dizin oluşturulmasını içerir. Dizin, bir kitabın arkasındaki dizin gibi, çeşitli bloklara işaretçiler içerir. Dosyadaki bir kaydı bulmak için önce dizinde arama yaparız ve ardından dosyaya doğrudan erişmek ve istenen kaydı bulmak için işaretçiyi kullanırız.

---

Bölüm: Koruma ve Güvenlik (Protection and Security)
...
Bölüm Detayı: Güvenlik Güncelleştirmeleri
