...
title: Threat Hunting with WAF Writeup - LetsDefend
type: writeups
---
...
* **Machine Learning Support:** Behavior-based systems are often supported by machine learning algorithms. This allows the WAF to better differentiate between normal and abnormal behavior over time, and to more accurately detect attacks.
...
* **Customized Reports:** The WAF can generate detailed reports on attack activity over a specified time period. Attack types, frequency, and sources are included in these reports.
...
During the threat hunting process, threat hunters create hypotheses to test specific attack vectors or the motivations of attackers. WAF provides data that can be used to test these hypotheses. For example, repeated attack attempts from a specific IP address or unusual traffic spikes can help threat hunters validate their hypotheses.
...
> **Earlier in the Threat Hunting process, an IP address performing Directory Traversal attacks was identified. Another IP address from the same threat group was found later. What was the firewall action on the endpoint's access request to this external IP address?**
