...
Bölüm Detayı: OffSec (Offensive Security) İlişkisi
...
Bölüm Detayı: Windows Subsystem for Linux (WSL) ile Kali Linux
...
Bölüm Detayı: WSL Kurulum Aşamaları:
...
Kurulum tamamlandıktan sonra bilgisayarınızı yeniden başlatın. İlk açılışta terminal sizden yeni bir kullanıcı adı ve parola belirlemenizi isteyecektir.


Bölüm Detayı: Grafik Arayüz (GUI) Kurulumu: Win-KeX
...
[Kod Bloğu: Burada bir kod örneği yer almaktadır. Kod içeriği seslendirmede atlanmıştır.]
Kurulum tamamlandıktan sonra grafik arayüzü başlatmak için şu komutu girin:.
[Kod Bloğu: Burada bir kod örneği yer almaktadır. Kod içeriği seslendirmede atlanmıştır.]
Bu komutla birlikte Windows içinde akıcı ve entegre bir Kali Linux masaüstü ortamına kavuşacaksınız.


Bölüm Detayı: Sanal Makine ile Lab Kurulum Aşamaları
...
Sistem mimarinize uygun sanallaştırma platformunu (VMware Workstation veya VirtualBox) ana bilgisayarınıza kurun.
Hazır (Pre-built) VM İmajını İndirme:.
Resmi web sitesindeki Kali Virtual Machines sayfasına giderek kullandığınız hipervizöre uygun (VMware için .7z veya VirtualBox için .ova uzantılı) hazır kalıbı indirin. Bu yöntem sizi disk yapılandırma zahmetinden kurtarır.
İmajı İçe Aktarma (Import) ve Donanım Ayarları:.
...
Bölüm Detayı: En Kritik Kali Linux Araçları Karşılaştırması
...
[Tablo Başlangıcı]
Kategori: Bilgi Toplama (Recon). Araç İsmi: nmap. Kullanım Amacı / Öne Çıkan Özellik: Ağ ve port tarama, servis/işletim sistemi tespiti. Güçlü Yönü / Tercih Sebebi: Hızlı, script desteği (NSE) çok güçlü. Alternatifi: masscan, rustscan.
Kategori: Zafiyet Analizi. Araç İsmi: nikto. Kullanım Amacı / Öne Çıkan Özellik: Web sunucu taraması, bilinen açıkları arama. Güçlü Yönü / Tercih Sebebi: Hızlı ve doğrudan web sunucusu hedefler. Alternatifi: Legion, Nessus.
Kategori: Web Uygulamaları. Araç İsmi: burpsuite. Kullanım Amacı / Öne Çıkan Özellik: Trafik izleme, araya girme (Proxy), istek manipülasyonu. Güçlü Yönü / Tercih Sebebi: Manuel testler için sektör standardı. Alternatifi: OWASP ZAP.
Kategori: Veritabanı Güvenliği. Araç İsmi: sqlmap. Kullanım Amacı / Öne Çıkan Özellik: SQL Injection açıklarını tespit ve sömürme. Güçlü Yönü / Tercih Sebebi: Otomatik exploit etme ve veriyi çekme. Alternatifi: Manuel SQLi.
Kategori: Parola Saldırıları. Araç İsmi: hashcat. Kullanım Amacı / Öne Çıkan Özellik: GPU destekli hızlı hash kırma. Güçlü Yönü / Tercih Sebebi: Çok yüksek hızlarda hash çözme yeteneği. Alternatifi: John the Ripper.
Kategori: İstismar (Exploitation). Araç İsmi: metasploit. Kullanım Amacı / Öne Çıkan Özellik: Hazır exploit modülleri çalıştırma. Güçlü Yönü / Tercih Sebebi: Geniş exploit ve payload kütüphanesi. Alternatifi: Cobalt Strike.
Kategori: Ağ İzleme (Sniffing). Araç İsmi: wireshark. Kullanım Amacı / Öne Çıkan Özellik: Ağ paketlerini detaylı yakalama ve analiz etme. Güçlü Yönü / Tercih Sebebi: Protokol düzeyinde derin paket çözümleme. Alternatifi: tcpdump.
Kategori: Sızma Sonrası (Post-Exploitation). Araç İsmi: NetExec (nxc). Kullanım Amacı / Öne Çıkan Özellik: AD ve Windows ağlarında yetki yayma / doğrulama. Güçlü Yönü / Tercih Sebebi: Eski crackmapexec yerine geçen modern AD aracı. Alternatifi: Impacket.
[Tablo Bitişi]
...
Bölüm Detayı: Ofansif Siber Saldırı Yaşam Döngüsü (Cyber Kill Chain) Uygulaması
...
Bölüm Detayı: Aşama: Keşif (Reconnaissance)
...
Bölüm Detayı: Aşama: İstismar (Exploitation)
...
Bölüm Detayı: Aşama: Sızma Sonrası (Post-Exploitation)
...
Bölüm Detayı: Araç Detayları ve Kullanım Örnekleri



Bölüm Detayı: Information Gathering
...
> Pratik Komut:
> ```bash
> nmap -sV -sC -Pn -oN scanresults.txt <HEDEFIP>
> ```
recon-ng: Recon-ng, Python'da yazılmış tam özellikli bir Web Keşif frameworküdür. Bağımsız modüller, veritabanı etkileşimi, etkileşimli yardım ve komut tamamlama özellikleriyle donatılan Recon-ng, web tabanlı keşiflerin hızlı ve kapsamlı bir şekilde yürütülebileceği güçlü bir ortam sağlar.
...
Bölüm Detayı: Vulnerability Analysis
...
Bölüm Detayı: Web Applicaton Analysis
...
Bölüm Detayı: Database Assesment
...
> ```
john: John the Ripper, sistem yöneticilerinin zayıf (tahmin etmesi veya kaba kuvvetle kırması kolay) parolaları bulmasına ve hatta istenirse kullanıcıları bu konuda otomatik olarak e-posta ile uyarmasına yardımcı olmak için tasarlanmış bir araçtır.
> Pratik Komut:
> ```bash
> john --wordlist=/usr/share/wordlists/rockyou.txt hash.txt
> ```
medusa: medusa'nın hızlı, büyük ölçüde paralel, modüler, kaba kuvvet saldırısı uygulaması olması amaçlanmıştır. Amaç, mümkün olduğu kadar uzaktan kimlik doğrulamaya izin veren çok sayıda hizmeti desteklemektir.
...
Bölüm Detayı: Reverse Engineering
...
Bölüm Detayı: Exploitation Tools


Bir hedef sistemi exploit etmek yani hedef sisteme erişim elde etmek için kullanılan araçlar bu bölümde yer alır. Bunlar:

NetExec (nxc): Eski adıyla crackmapexec olarak bilinen aracın geliştirilmesi durdurulduktan (deprecated) sonra, topluluk tarafından sürdürülen ve geliştirilmeye devam eden modern sürümüdür. Windows/Active Directory ortamlarında sızma testleri, kimlik doğrulama doğrulaması ve dikey/yatay yetki yayma işlemleri için vazgeçilmez bir araçtır.
> Pratik Komut:
> ```bash
> nxc smb <HEDEFIPVEYA_AG> -u username -p password --local-auth
> ```
metasploit framework: Metasploit framework, güvenlik açığı araştırmasını, istismar yöntemi geliştirmeyi ve özel güvenlik araçlarının oluşturulmasını destekleyen açık kaynaklı bir platformdur.
searchsploit: Exploit Veritabanında hazır exploit araması yapar.


Bölüm Detayı: Sniffing&Spoofing
...
Bölüm Detayı: Post Exploitation
...
proxychains4: Proxychains, ağla ilgili libc işlevlerini önceden yüklenmiş bir DLL (dlsym(), LD\_PRELOAD) aracılığıyla dinamik olarak bağlı programlara bağlayan ve bağlantıları SOCKS4a/5 veya HTTP proxy'leri aracılığıyla yeniden yönlendiren bir UNIX programıdır. Yalnızca TCP'yi destekler (UDP/ICMP vb. yoktur).
...
bulk\extractor: bulk\extractor, bir disk görüntüsünü, bir dosyayı veya bir dosya dizinini tarayan ve dosya sistemi veya dosya sistemi yapılarını ayrıştırmadan faydalı bilgileri çıkaran bir C++ programıdır. Sonuçlar, kolayca incelenebilen, ayrıştırılabilen veya otomatik araçlarla işlenebilen özellik dosyalarında saklanır.
...
Bölüm: Geleceğin Ofansif Güvenliği: Yapay Zeka ve Otomasyon
