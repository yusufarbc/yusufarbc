...
class BlogManager {
    ...
    processArticles(items) {
        return items.map(item => ({
            ...
            views: Math.floor(Math.random() * 2000) + 500, // Simulated
            likes: Math.floor(Math.random() * 100) + 20 // Simulated
        }));
    }
    ...
    createArticleCard(article) {
        ...
        card.innerHTML = `
            <div class="card h-100 border-0 shadow-sm blog-card" onclick="window.open('${article.link}', '_blank')">
                <div class="card-body">
    ...
    showFallbackContent() {
        ...
        // Show load more button for fallback content
        const loadMoreBtn = document.getElementById('load-more-btn');
        if (loadMoreBtn) {
            loadMoreBtn.style.display = 'block';
        }
